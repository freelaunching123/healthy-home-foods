...
class _FruitCartScreenState extends State<FruitCartScreen> {
  ...
  Future<void> _updateQty(Map<String, dynamic> item, double newQty) async {
    ...
    try {
      await _api.put('${ApiConstants.fruitCart}/${item['id']}', data: {'quantity_kg': newQty});
      _loadCart();
    } catch (e) {
      if (mounted) {
  ...
  Future<void> _removeItem(Map<String, dynamic> item) async {
    try {
      await _api.delete('${ApiConstants.fruitCart}/${item['id']}');
      _loadCart();
    } catch (e) {
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
  ...
  Future<void> _clearCart() async {
    ...
    try {
      await _api.delete(ApiConstants.fruitCart);
      _loadCart();
    } catch (_) {
    } finally {
        ...
        children: [
          ...
          Expanded(
            child: ElevatedButton(
              onPressed: () => context.push('/fruits/checkout'),
  ...
  Widget _buildEmpty() => Center(
    child: Column(
      ...
      children: [
        ...
        Text('Add some fresh fruits to get started!', style: GoogleFonts.inter(color: AppTheme.textSecondary)),
