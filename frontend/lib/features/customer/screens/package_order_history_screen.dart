...
class PackageOrderHistoryScreen extends StatefulWidget {
  const PackageOrderHistoryScreen({super.key});

  @override
  State<PackageOrderHistoryScreen> createState() => _PackageOrderHistoryScreenState();
}

class _PackageOrderHistoryScreenState extends State<PackageOrderHistoryScreen> {
  ...
  Future<void> _loadOrders() async {
    ...
    try {
      final res = await _api.get(ApiConstants.packageOrdersHistory);
      setState(() {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        title: Text('My package Orders', style: GoogleFonts.inter(fontWeight: FontWeight.w700)),
              ...
              : _orders.isEmpty
                  ...
                  : ListView.separated(
                        ...
                        itemBuilder: (ctx, i) => _OrderCard(
                          order: _orders[i],
                          onTap: () => context.push('/profile/package-history/${_orders[i]['id']}'),
                        ),
                      ),
    );
  }
...
class _OrderCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final itemSummary = items.take(3).map((i) {
      ...
      final name = i['product'] != null ? i['product']['name'] : i['product_name'] ?? 'Package';
      return '$name ($qty)';
    }).join(', ');
